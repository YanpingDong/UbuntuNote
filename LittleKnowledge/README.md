...
网桥和集线器（转发器）的一个重要区别：网桥是按**存储转发方式**工作的，一定是先把整个帧收下来（集线器转发器是逐比特转发）在进行处理。网桥是通过内部的接口管理软件和网桥协议实体来完成上述操作的。好处如下： 1. 过滤通信量，增大吞吐量。
...
- 增加时延。由于需要网桥进行存储转发，所以增加了时延。
- 在MAC层并没有流量控制功能。当网络符合过重，网桥中的缓存的存储空间会因为不够而溢出。
...
在收到数据包时，路由器会进行如下操作：
    ...
    路由器在收到一个包时，从包头部分提取出目的 IP 地址。
    在自己的全局路由表中查找是否有此 IP 的路由信息。若没有，此将此包发往默认路由。若有，则将此包发送到路由表中记录的相应的下一跳处。

通过路由实现的全网通信示例

![](pic/RouterNetCumunication.png)

1. 要实现全网通信，也就是网络中任意两个节点都可以通信，就要求每个路由器的路由表中必须有到所有网段的路由。如上图所示，R1在链接时就知道A网和B网，但要让R1知道B网怎么到的C网和C网怎么到的D网。所以要做两个配置。
